...
def main(args):
    data_train = NlpData(os.path.join(DIR, args.data_dir), args.file_name, args.vocab_size)
...
if __name__ == '__main__':
    ...
    if args.predef == 1:
        ...
        if len(list(filter(lambda x: x is None, vars(args).values()))) > 1 and args.predef is None:
