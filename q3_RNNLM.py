...
class Config(object):
    ...
    hidden_size = 100
    num_steps = 10
    max_epochs = 1 if _DEBUG else 16
    early_stopping = 2
    dropout = 0.9
    lr = 0.001
...
class RNNLM_Model(LanguageModel):
    ...
    def add_embedding(self):
        ...
        with tf.device('/cpu:0'):
            ...
            L = tf.Variable(tf.random_uniform(Lshape, -1.0, 1.0))
            embeds = tf.nn.embedding_lookup(L, self.input_placeholder)
            steps = tf.split(embeds, self.config.num_steps, 1)
            inputs = [tf.squeeze(i, [1]) for i in steps]
            ### END YOUR CODE
            return inputs
    ...
    def add_loss_op(self, output):
        ...
        """
        ### YOUR CODE HERE
        # output is shape (dataset x vocabulary)
        # self.outputs is list of num_batches (batch_size x vocabulary)
        # not sure exactly what type of loss they are looking for...
    ...
    def add_model(self, inputs):
        ...
        # apply dropout on inputs -- Should this be in a variable scope??
        dropped_inputs = [tf.nn.dropout(i, self.dropout_placeholder) for i in inputs] 
        context = list()
        with tf.variable_scope("RNN") as scope:
            # declare shapes
            Hshape = (self.config.hidden_size, self.config.hidden_size)
            Ishape = (self.config.embed_size, self.config.hidden_size)
            b1shape = (self.config.hidden_size, )
            # initialize weights
            H = tf.get_variable("H", shape=Hshape, initializer=xinit)
            I = tf.get_variable("I", shape=Ishape, initializer=xinit)
            b1 = tf.get_variable("b1", shape=b1shape, initializer=xinit)

            context.append(self.initial_state)
            for indx in range(self.config.num_steps):
                if indx > 0: scope.reuse_variables()
                hidden_state = tf.matmul(dropped_inputs[indx], I) +\
                    tf.matmul(context[-1], H) + b1
                context.append(tf.nn.sigmoid(hidden_state))
            # store final state
            self.final_state = context[-1]
            rnn_outputs = context[1:]
