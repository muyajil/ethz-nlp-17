...
class NERModel(LanguageModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(tf.int32,
            shape=(None, self.config.window_size), name='input')
    ...
    def add_embedding(self):
        ...
        with tf.device('/cpu:0'):
            ### YOUR CODE HERE
            L = tf.Variable(tf.zeros([len(self.wv), self.config.embed_size]), name='embed')
    ...
    def add_model(self, window):
        ...
        with tf.variable_scope("Layer"):
            Wshape = (self.config.window_size * self.config.embed_size, self.config.hidden_size)
            self.W = tf.get_variable("weights", shape=Wshape, initializer=xavier_weight_init())
            self.b1 = tf.get_variable("bias", shape=[self.config.hidden_size], initializer=xavier_weight_init())
        ...
        with tf.variable_scope("Softmax"):
            ...
            self.U = tf.get_variable("weights", shape=Ushape, initializer=xavier_weight_init())
            self.b2 = tf.get_variable("bias", shape=[self.config.label_size], initializer=xavier_weight_init())
...
#        output = tf.nn.dropout(output, self.dropout_placeholder)
    ...
    def add_loss_op(self, pred):
        ...
        wreg = self.config.l2 * tf.nn.l2_loss(self.W)
        ureg = self.config.l2 * tf.nn.l2_loss(self.U)
        loss = loss + wreg + ureg
        ### END YOUR CODE
        return loss
